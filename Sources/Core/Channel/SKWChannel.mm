...
public:
    ...
    void OnMemberMetadataUpdated(std::shared_ptr<skyway::core::interface::Member> nativeMember,
        ...
        if ([strongChannel.delegate respondsToSelector:@selector(channel:
                                                                  member:metadataDidUpdate:)]) {
            NSString* metadata = [NSString stringForStdString:nativeMetadata];
            id member          = [strongChannel.repository findMemberByMemberID:nativeMember->Id()];
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                  [strongChannel.delegate channel:strongChannel
                                           member:member
    ...
    void OnPublicationListChanged() override {
        ...
        if ([strongChannel.delegate
                respondsToSelector:@selector(channelPublicationListDidChange:)]) {
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    void OnStreamPublished(
        ...
        if ([strongChannel.delegate respondsToSelector:@selector(channel:
            ...
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                  [strongChannel.delegate channel:strongChannel
                      didPublishStreamOfPublication:publication];
                });
        }
    }
    void OnStreamUnpublished(
        ...
        id publication =
            [strongChannel.repository findPublicationByPublicationID:nativePublication->Id()];
        if ([strongChannel.delegate respondsToSelector:@selector(channel:
                                                           didUnpublishStreamOfPublication:)]) {
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                  [strongChannel.delegate channel:strongChannel
                      didUnpublishStreamOfPublication:publication];
                });
        }
    }
    ...
    void OnPublicationEnabled(
        ...
        if ([strongChannel.delegate respondsToSelector:@selector(channel:
                                                           publicationDidChangeToEnabled:)]) {
            id publication =
                [strongChannel.repository findPublicationByPublicationID:nativePublication->Id()];
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                  [strongChannel.delegate channel:strongChannel
                      publicationDidChangeToEnabled:publication];
                });
        }
    }
    ...
    void OnPublicationDisabled(
        ...
        if ([strongChannel.delegate respondsToSelector:@selector(channel:
                                                           publicationDidChangeToDisabled:)]) {
            id publication =
                [strongChannel.repository findPublicationByPublicationID:nativePublication->Id()];
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                  [strongChannel.delegate channel:strongChannel
                      publicationDidChangeToDisabled:publication];
                });
        }
    }
    ...
    void OnSubscriptionListChanged() override {
        ...
        if ([strongChannel.delegate
                respondsToSelector:@selector(channelSubscriptionListDidChange:)]) {
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    void OnPublicationSubscribed(
        ...
        id subscription =
            [strongChannel.repository registerSubscriptionIfNeeded:nativeSubscription];
        if ([strongChannel.delegate
                respondsToSelector:@selector(channel:didSubscribePublicationOfSubscription:)]) {
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                  [strongChannel.delegate channel:strongChannel
                      didSubscribePublicationOfSubscription:subscription];
                });
        }
    }
    void OnPublicationUnsubscribed(
        ...
        id subscription =
            [strongChannel.repository findSubscriptionBySubscriptionID:nativeSubscription->Id()];
        if ([strongChannel.delegate
                respondsToSelector:@selector(channel:didUnsubscribePublicationOfSubscription:)]) {
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                  [strongChannel.delegate channel:strongChannel
                      didUnsubscribePublicationOfSubscription:subscription];
                });
        }
    }
...
- (SKWLocalPerson* _Nullable)localPerson {
    ...
    [_repository.members
        enumerateObjectsUsingBlock:^(SKWMember* _Nonnull obj, NSUInteger idx, BOOL* _Nonnull stop) {
...
- (void)joinWithInit:(SKWMemberInit*)init
