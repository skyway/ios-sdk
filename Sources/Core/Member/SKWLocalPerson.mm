...
public:
    ...
    void OnStreamPublished(NativePublication* nativePublication) override {
        if ([person_.delegate respondsToSelector:@selector(localPerson:
            ...
            // the event of LocalPerson comes earlier than channel's event
    ...
    void OnStreamUnpublished(NativePublication* nativePublication) override {
        if ([person_.delegate respondsToSelector:@selector(localPerson:
            ...
            id publication =
                [person_.repository findPublicationByPublicationID:nativePublication->Id()];
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                  [person_.delegate localPerson:person_
    ...
    void OnPublicationSubscribed(NativeSubscription* nativeSubscription) override {
        if ([person_.delegate respondsToSelector:@selector(localPerson:
            ...
            // because the event of LocalPerson comes earlier than channel's event
    ...
    void OnPublicationUnsubscribed(NativeSubscription* nativeSubscription) override {
        if ([person_.delegate respondsToSelector:@selector(localPerson:
            ...
            id subscription =
                [person_.repository findSubscriptionBySubscriptionID:nativeSubscription->Id()];
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                  [person_.delegate localPerson:person_
...
- (void)subscribePublicationWithPublicationID:(NSString* _Nonnull)publicationID
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      auto nativeSubscription =
          nativePerson->Subscribe(nativePublicationId, options.nativeSubscriptionOptions);
      if (completion) {
          if (nativeSubscription) {
              SKWSubscription* sub =
                  [self.repository registerSubscriptionIfNeeded:nativeSubscription];
              completion(sub, nil);
          } else {
