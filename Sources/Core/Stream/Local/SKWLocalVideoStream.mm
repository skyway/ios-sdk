...
- (id _Nonnull)initWithSource:(SKWVideoSource* _Nonnull)source
    ...
    if (self = [super initWithSharedNative:native]) {
        ...
        [[NSNotificationCenter defaultCenter]
            addObserver:self
               selector:@selector(customFrameUpdated:)
                   name:NSString.SKWCustomFrameVideoSourceDidUpdateFrame
                 object:nil];
    }
    return self;
}
...
- (void)customFrameUpdated:(NSNotification* _Nonnull)notification {
    ...
    RTCVideoFrame* frame = [[notification userInfo]
        objectForKey:NSString.SKWCustomFrameVideoSourceDidUpdateFrameKeyForFrame];
