...
-(AVCaptureDeviceFormat*)selectFormatForCapturer:(RTCCameraVideoCapturer* _Nonnull)capturer maxWidth:(int)maxWidth maxHeight:(int)maxHeight;
...
-(void)startCapturingWithDevice:(AVCaptureDevice* _Nonnull)device
    ...
    RTCCameraVideoCapturer* cameraCapturer = (RTCCameraVideoCapturer*)self.rtcCapturer;
    AVCaptureDeviceFormat* format = [self selectFormatForCapturer:cameraCapturer maxWidth:options.preferredMaxWidth maxHeight:options.preferredMaxHeight];
    NSInteger fps = [self selectFpsForFormat:format preferredMaxFrameRate:options.preferredMaxFrameRate];
    [cameraCapturer startCaptureWithDevice:_camera format:format fps:fps completionHandler:completion];
}
...
-(BOOL)isSupportedCamera:(AVCaptureDevice* _Nonnull)camera {
    NSUInteger idx = [SKWCameraVideoSource.supportedCameras indexOfObjectPassingTest:^BOOL(AVCaptureDevice * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        return obj == camera;
    }];
