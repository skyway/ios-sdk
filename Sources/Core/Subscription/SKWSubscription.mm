...
public:
    ...
    void OnConnectionStateChanged(const skyway::core::ConnectionState new_state) override {
        ...
        if ([subscription_.delegate respondsToSelector:@selector(subscription:
                                                           connectionStateDidChange:)]) {
            dispatch_group_async(
                group_, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                  [subscription_.delegate subscription:subscription_
                              connectionStateDidChange:state];
                });
        }
    }
...
- (SKWSubscriptionState)state {
    switch (_native->State()) {
        case skyway::core::interface::SubscriptionState::kEnabled:
            return SKWSubscriptionStateEnabled;
        case skyway::core::interface::SubscriptionState::kDisabled:
            return
                // TODO: Change it to disabled
                SKWSubscriptionStateCanceled;
        case skyway::core::interface::SubscriptionState::kCanceled:
            return SKWSubscriptionStateCanceled;
    }
}
    ...
    switch (type) {
        ...
        case skyway::model::ContentType::kData:
            _stream = [[SKWRemoteDataStream alloc] initWithSharedNative:nativeStream
                                                             eventGroup:_repository.eventGroup];
            break;
    }
}
